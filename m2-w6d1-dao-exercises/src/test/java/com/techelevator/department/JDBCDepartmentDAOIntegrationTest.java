...
public class JDBCDepartmentDAOIntegrationTest {
	...
	public void testingCreateDepartmentMethod() {
		...
		Department deptObject1 = createDepartmentObject("Test Department 100", longVar); // This creates a Department
																							// Object

		dao.createDepartment(deptObject1); // This (should) create the new department, it also somehow returns
											// deptObject1 back, with it's auto_generated id_number.
	...
	// this tests the saveDepartment() method which simply updates the name of a
	// department
	@Test
	public void testSaveDepartment() {
		...
		dao.createDepartment(deptObject1); // Create a new department, to get their id, then change their name and test
											// it

		deptObject1.setName("Not Test Dept 200"); // Switch the name of deptObject1

		dao.saveDepartment(deptObject1); // Resave it. The Id gets created returned and set when the dept object is
											// saved in the database

		Department resultantDept = dao.getDepartmentById(deptObject1.getId()); // Make a copy of the dept that we just
																				// "updated the name"
	...
	public void testGetAllDepartments() {
		// Going to create a new dept, then getAllDepts() and make sure it's in the
		// list, but not the only one in the list
	...
	public void testGetDepartmentsByName() {
		...
		for (Department dept : listOfDepartments) {
			...
			if (dept.getName().contains(deptObject1.getName())) { // if the name matches
				...
				if (dept.getId() == deptObject1.getId()) { // if the id number matches. The id number should only match
															// once, even if there are multiple matches on the name
