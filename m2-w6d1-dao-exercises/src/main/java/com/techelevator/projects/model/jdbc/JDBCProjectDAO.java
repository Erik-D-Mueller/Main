...
public class JDBCProjectDAO implements ProjectDAO {
	...
	while(results.next()) {
		...
		if(tempProjectObject.getStartDate()!=null && tempProjectObject.getStartDate().compareTo(currentDate) < 0) {		
			
			if(tempProjectObject.getEndDate()==null || tempProjectObject.getEndDate().compareTo(currentDate) > 0) {
	...
	private Project createProjectObject(SqlRowSet results){
		...
		// If we try to call .toLocalDate() on a null value, it ultimately causes a nullPointerException and crashes the program
