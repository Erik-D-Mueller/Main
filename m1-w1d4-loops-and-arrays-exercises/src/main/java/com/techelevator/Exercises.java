...
public class Exercises {
	...
	//What makes this problem tricky is that we have to "round the corner" which means the last value gets inserted into the first element spot.
	...
	public int[] rotateLeft3(int[] nums) {
		...
		temp = nums[0];  // Grab the value of the first element and store it seperately
		
		for(int i = 0; i <= nums.length-2; i++) 
		{	
			nums[i]=nums[i+1];  //shift all the middle elements down by one (except the first and last)
		}
		
		nums[nums.length-1]=temp; //Grab the seperatley stored first element value and tack it on the very end
								//The loop essentailly shifts all the middle values, and the statements just before and after the loop "round the corner".
		return nums;
	...
	public int[] reverse3(int[] nums) {
		...
		//int backward[nums.length];
		
		//int[5] backward;
		
		int[] backward;
		backward = new int[nums.length];  
		
		// For some reason, in java, we declare the length of the array seperately.
		// The above sets the length of backward[] to be the same as nums[].
		
		for(int i =0; i<nums.length; i++) {
			
			backward[i]=nums[nums.length - (i+1)];		
	...
	public int sum13(int[] nums) {
		...
		if(nums[0]!=13)	{sum+=nums[0];}  // on the loop I have to check the previous elements value as well, but that won't work for the very first element.
